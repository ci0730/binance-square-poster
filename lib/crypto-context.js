...
export async function fetchCryptoNews({ limit = 6 } = {}) {
  ...
  const results = await Promise.all(
    RSS_FEEDS.map(async (feed) => {
      try {
        const res = await fetchText(feed.url, 5000);
        if (res.status !== 200) return { error: `${feed.name}: HTTP ${res.status}`, items: [] };
        return {
          error: null,
          items: parseRss(res.text, limit).map((item) => ({ ...item, source: item.source || feed.name })),
        };
      } catch (err) {
...
const FUTURES_TICKER_CACHE_MS = 20 * 1000;
...
export async function buildCryptoContext({
  ...
  if (explicit.length > 0) {
    ...
    [news, tickersResult] = await Promise.all([
      ...
      fetchMarketTickers(tickerSymbols),
    ]);
  } else {
  ...
  const { tickers, missing: missingTickers } = tickersResult;
