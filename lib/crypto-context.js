...
let alphaTokenCache = { at: 0, map: new Map() };
...
async function fetchAlphaTokenMap() {
  if (Date.now() - alphaTokenCache.at < ALPHA_CACHE_MS && alphaTokenCache.map.size) {
    return alphaTokenCache.map;
  }
  ...
  alphaTokenCache = { at: Date.now(), map };
  return map;
}
...
async function fetchAlphaTicker(symbol) {
  const map = await fetchAlphaTokenMap();
...
async function fetchTokenQuote(symbol) {
  ...
  // 自定义代币优先走币安 Alpha，避免与现货同名标的混淆
  if (!isMainstream(normalized)) {
    const alpha = await fetchAlphaTicker(normalized);
    if (alpha) return alpha;
  }
  const spot = await fetchSpotTicker(normalized);
  if (spot) return spot;
  return fetchAlphaTicker(normalized);
}
...
export function formatContextForPrompt(context) {
  ...
  if (context.tickers?.length) {
    lines.push("【24h 行情（币安真实数据，正文只能使用以下价格）】");
    context.tickers.forEach((t) => {
      ...
      lines.push(
        `${t.symbol} (${marketLabel}): $${priceText} USDT (24h ${changeText})` +
          (t.tokenName && t.tokenName !== t.symbol ? `，项目名 ${t.tokenName}` : "") +
          (t.chainName ? `，链 ${t.chainName}` : ""),
      );
    });
    lines.push(
      "【价格纪律】以上价格为币安实时报价（含 Alpha 代币）。正文引用价格时必须与上述数字一致，禁止编造、猜测、凭记忆填写，也不要与其他同名项目混淆。",
    );
  }
