 ...
 * 解析指定账号实际发帖用的代理 URL。
 * - direct：直连（空字符串）
 * - global：走软件全局/本机 Clash
 * - http/https/socks5/ssh：走该账号独立 IP（buildProxyUrl）
 * 自定义代理 host/port 不完整时回落全局，避免拼出无效 URL。
 */
...
export function mergeAccountProxyCredentials(accountId, config) {
  ...
  if (!accountId || !isCustomProxyConfig(normalized)) {
    return normalized;
  }
  // 用户明确输入了新密码时用新密码；留空/掩码则回填已保存密码
  if (!isBlankProxyPassword(normalized.password)) {
    return normalized;
  }
  const account = getAccount(accountId);
  if (!account) return normalized;
  const stored = normalizeProxyConfig(account.proxyConfig, account.proxy);
...
export function updateAccount(accountId, patch) {
  ...
  if (patch.proxyConfig != null) {
    ...
    if (isBlankProxyPassword(next.password)) {
      const stored = normalizeProxyConfig(account.proxyConfig, account.proxy);
      if (!isBlankProxyPassword(stored.password)) {
