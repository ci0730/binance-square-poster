...
/** 从 chat completions URL 推导 /v1/models */
export function toOpenAiModelsUrl(chatCompletionsUrl = "") {
  const raw = String(chatCompletionsUrl || "").trim().replace(/\/+$/, "");
  if (!raw) return "";
...
/** 中转站/Codex 常用候选（用户输入优先，再按此列表试） */
export const AUTO_MATCH_MODEL_CANDIDATES = [
  "gpt-4o-mini",
  "gpt-4o",
  "gpt-4.1-mini",
  "gpt-4.1",
...
export function resolveProviderRuntime({ provider: providerId, model, baseUrl = "", allowEmptyModel = false } = {}) {
  const provider = getAiProvider(providerId);
  ...
  if (!resolvedModel && !allowEmptyModel) {
