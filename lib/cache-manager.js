...
import { clearSystemLogs, getSystemLogFileInfo } from "./system-log.js";
...
export function getCacheOverview(appRoot = "") {
  ...
  const logInfo = getSystemLogFileInfo();
  ...
  const cacheItems = [
    ...
    },
    {
      id: "logs",
      label: "运行日志",
      description: "本地持久化的系统/托管日志，关闭软件后仍保留；清理后才会消失",
      detail: `${logInfo.count} 条`,
      bytes: logInfo.bytes || 0,
      sizeLabel: formatBytes(logInfo.bytes || 0),
      clearable: true,
      checked: false,
    },
  ];
