...
function clashHttpOverPipe(pipePath, reqPath, { method = "GET", body = null, timeoutMs = 3000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const tryParse = () => {
      ...
      // 无 Content-Length 时等连接结束再解析（部分 PUT 响应）
      return false;
    };
...
async function clashHttpRequest(base, reqPath, { method = "GET", body = null, timeoutMs = 3000 } = {}) {
  if (base.kind === "pipe") {
    return clashHttpOverPipe(base.path, reqPath, { method, body, timeoutMs });
  }
  const headers = {
    Accept: "application/json",
  };
  if (base.secret) headers.Authorization = `Bearer ${base.secret}`;
...
async function clashHttpGet(base, reqPath, { timeoutMs = 3000 } = {}) {
  return clashHttpRequest(base, reqPath, { method: "GET", timeoutMs });
}
...
const DENY_OUTBOUNDS = new Set(["DIRECT", "REJECT", "REJECT-DROP", "PASS", "COMPATIBLE"]);

function walkClashNowChain(proxies, startName) {
  let name = String(startName || "").trim();
  if (!name || DENY_OUTBOUNDS.has(name)) return "";
  for (let depth = 0; depth < 8; depth += 1) {
    const node = proxies[name];
    if (!node?.now || DENY_OUTBOUNDS.has(node.now)) break;
    name = node.now;
  }
  if (!name || DENY_OUTBOUNDS.has(name)) return "";
  return name;
...
/**
 * 找可手动切换的 Selector 分组（不对 URLTest「自动选择」强行改 now）。
 * @returns {{ group: string, current: string, all: string[] } | null}
 */
export function findClashSwitchableSelector(proxies) {
  ...
  const tryGroup = (groupName) => {
    const node = proxies[groupName];
    if (!node || String(node.type || "") !== "Selector") return null;
    const all = (Array.isArray(node.all) ? node.all : [])
      .map((n) => String(n || "").trim())
      .filter((n) => n && !DENY_OUTBOUNDS.has(n) && !isClashGroupType(proxies[n]?.type));
    if (!all.length) return null;
    const current = String(node.now || "").trim();
    return { group: groupName, current, all };
  };

  for (const key of preferred) {
    const hit = tryGroup(key);
    if (hit) return hit;
  }

  // 非 GLOBAL 的其它 Selector
  for (const [name, node] of Object.entries(proxies)) {
    if (name === "GLOBAL") continue;
    if (String(node?.type || "") !== "Selector") continue;
    const hit = tryGroup(name);
    if (hit) return hit;
  }

  // 最后 GLOBAL（部分配置把节点直接挂在 GLOBAL）
  return tryGroup("GLOBAL");
}

/**
 * 从 Selector 候选里挑出可测速切换的节点（排除当前与黑名单）。
 */
export function pickClashFailoverCandidates(selector, { excludeNames = [] } = {}) {
  if (!selector?.all?.length) return [];
  const deny = new Set(
    [...excludeNames, selector.current].map((n) => String(n || "").trim()).filter(Boolean),
  );
  return selector.all.filter((name) => !deny.has(name));
}
...
async function loadClashProxiesFromBase(base, timeoutMs = 1200) {
  const listRes = await clashHttpGet(base, "/proxies", { timeoutMs });
  if (listRes.status === 401 || listRes.status === 403) return null;
  if (listRes.status < 200 || listRes.status >= 300) return null;
  try {
    return JSON.parse(listRes.body)?.proxies || null;
  } catch {
    return null;
  }
}

async function findWorkingClashController(timeoutMs = 1200) {
  const bases = clashControllerBases();
  for (const base of bases.filter((b) => b.kind === "pipe")) {
    try {
      const proxies = await loadClashProxiesFromBase(base, timeoutMs);
      if (proxies) return { base, proxies };
    } catch {
      // next
    }
  }
  for (const base of bases.filter((b) => b.kind === "tcp")) {
    try {
      const proxies = await loadClashProxiesFromBase(base, Math.min(timeoutMs, 1000));
      if (proxies) return { base, proxies };
    } catch {
      // next
    }
...
async function measureNodeDelayOnBase(base, nodeName, timeoutMs) {
  const ms = Math.max(1500, Math.min(Number(timeoutMs) || 4000, 6000));
  const testUrl = encodeURIComponent("http://www.gstatic.com/generate_204");
  try {
    const delayRes = await clashHttpGet(
      base,
      `/proxies/${encodeURIComponent(nodeName)}/delay?timeout=${ms}&url=${testUrl}`,
      { timeoutMs: ms + 1200 },
    );
    if (delayRes.status < 200 || delayRes.status >= 300) return null;
    const delay = Number(JSON.parse(delayRes.body)?.delay);
    if (Number.isFinite(delay) && delay > 0) return delay;
  } catch {
    // ignore
  }
...
/**
 * 发帖前预检失败时：在 Clash Selector 组里自动切到延迟更好的可用节点。
 * 控制面不可用或无可切节点时返回 null（不抛错）。
 * @returns {Promise<{ from: string, to: string, group: string, latencyMs: number|null } | null>}
 */
export async function tryFailoverClashNode({ excludeNames = [], timeoutMs = 5000, maxCandidates = 6 } = {}) {
  const found = await findWorkingClashController(1200);
  if (!found) return null;
  const { base, proxies } = found;
  const selector = findClashSwitchableSelector(proxies);
  if (!selector) return null;

  const leaf = resolveClashLeafName(proxies) || selector.current;
  const candidates = pickClashFailoverCandidates(selector, {
    excludeNames: [...excludeNames, leaf],
  }).slice(0, Math.max(1, Math.min(Number(maxCandidates) || 6, 10)));
  if (!candidates.length) return null;

  const scored = [];
  for (const name of candidates) {
    const delay = await measureNodeDelayOnBase(base, name, timeoutMs);
    if (delay != null) scored.push({ name, delay });
  }
  // 测速都失败时，仍尝试切到第一个候选（部分节点 delay API 不稳定）
  const target = scored.sort((a, b) => a.delay - b.delay)[0]?.name || candidates[0];
  if (!target || target === selector.current) return null;

  const putRes = await clashHttpRequest(base, `/proxies/${encodeURIComponent(selector.group)}`, {
    method: "PUT",
    body: { name: target },
    timeoutMs: 3000,
  });
  if (putRes.status < 200 || putRes.status >= 300) return null;

  const latencyMs = scored.find((s) => s.name === target)?.delay ?? null;
  return {
    from: leaf || selector.current || "",
    to: target,
...
/** 轻量刷新 URLTest 组延迟（不改手动 Selector） */
export async function refreshClashUrlTestDelay({ timeoutMs = 5000 } = {}) {
  const found = await findWorkingClashController(1200);
  if (!found) return false;
  const { base, proxies } = found;
  for (const [name, node] of Object.entries(proxies)) {
    if (String(node?.type || "") !== "URLTest") continue;
    await measureNodeDelayOnBase(base, name, timeoutMs);
    return true;
  }
  return false;
}

let lastSeenClashNodeName = "";
...
export async function measureProxyLatency(proxyUrl = "", { timeoutMs = 5000 } = {}) {
  ...
  if (fromClash) {
    const nodeName = String(fromClash.nodeName || "");
    if (nodeName && lastSeenClashNodeName && nodeName !== lastSeenClashNodeName) {
      try {
        const { resetProxyCircuit } = await import("./proxy-circuit.js");
    ...
    if (nodeName) lastSeenClashNodeName = nodeName;
    return fromClash;
  }
