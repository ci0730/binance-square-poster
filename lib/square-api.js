...
async function publish(apiKey, body, proxyUrl = "", fallbackProxyUrl = "") {
  ...
  while (true) {
    try {
      ...
      if (is429 && rateLimitAttempt < 3) {
        rateLimitAttempt += 1;
        await sleep(1200 * rateLimitAttempt + Math.floor(Math.random() * 400));
        continue;
      }
      if (isDefinitelyUnsentPublishError(err) && unsentRetries < PUBLISH_UNSENT_RETRIES) {
        unsentRetries += 1;
        await sleep(900 * unsentRetries + Math.floor(Math.random() * 500));
        continue;
      }
...
export async function publishPost(apiKey, post, uploadsDir, onProgress, options = {}) {
  return withGlobalPublishQueue(async () => {
    ...
    if (!skipPreflight) {
      try {
        const ready = await assertPublishNetworkReady(effectiveProxy, { onProgress });
        if (ready?.failover?.to) {
          onProgress?.({
            stage: "network_failover",
            message: `已自动切换 VPN 节点：${ready.failover.from || "原节点"} → ${ready.failover.to}，继续发帖`,
          });
        }
      } catch (err) {
        ...
        if (canUseFallbackProxy(effectiveProxy, fallbackProxyUrl)) {
          ...
          const ready = await assertPublishNetworkReady(fallbackProxyUrl, { onProgress });
    ...
    try {
      const result = await publish(apiKey, body, effectiveProxy, fallbackProxyUrl);
      if (!result?.id) {
