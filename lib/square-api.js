  ...
  try {
    ...
    if (
      ...
      const proxyHint = proxy
        ...
        : "（国内网络通常无法直连币安，请在设置中配置代理，或开启梯子后使用系统代理）";
      throw new Error(`无法连接币安 API${proxyHint}`);
    }
    throw err;
  }
}
...
async function api(endpoint, apiKey, body, baseUrl = BASE_URL_V2, proxyUrl = "") {
...
export async function testApiKey(apiKey, { proxyUrl = "" } = {}) {
  await api("/image/presignedUrl", apiKey, { imageName: "test.png" }, BASE_URL_V2, proxyUrl);
  return true;
}
