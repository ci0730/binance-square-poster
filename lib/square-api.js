...
import { assertPublishNetworkReady, canUseFallbackProxy, shouldAllowProxyFallback, isRemoteAccountProxy } from "./proxy-health.js";
 ...
 * 无 Cookie 时跳过（私有列表/浏览器核对都依赖登录态），避免空转失败。
...
export async function confirmRecentPublish({
  ...
  if (!cookieStr) {
    onProgress?.({
      stage: "confirming",
      message: "未配置 Cookie，已跳过自动核对广场记录…",
    });
    try {
      appendSystemLog(
        "[发帖] 账号未配置 Cookie，已跳过依赖 Cookie 的广场核对（发帖本身不需要 Cookie）",
        { type: "info", source: "publish" },
      );
    } catch {
      // ignore
    }
...
function makeUncertainPublishError(cause, { hasCookie = true } = {}) {
  const uncertain = new Error(
    hasCookie
      ? "帖子可能已提交，但确认回包中断。软件已自动核对广场记录仍未确认；请到「已发布帖子」手动拉取一次，避免重复发帖。"
      : "帖子可能已提交，但确认回包中断。当前账号未配置 Cookie，已自动跳过广场核对；请到「已发布帖子」手动确认，避免重复发帖。配置 Cookie 后可在网络抖动时自动核对。",
  );
  uncertain.code = "PUBLISH_CONFIRMATION_UNKNOWN";
  uncertain.cause = cause;
  uncertain.skippedCookieConfirm = !hasCookie;
  return uncertain;
}

/**
 * 回包丢失、核对不到时，仍按「已发出」返回成功（测试/统计真实成功率用）。
 * 调用方应提示用户勿立刻重发；有 Cookie 时优先走 confirmRecentPublish。
 */
function makeAssumedPublishSuccess(cause, { hasCookie = true, text = "" } = {}) {
  const tip = hasCookie
    ? "未抓到成功回包且广场未核对到，已默认记为发布成功（请勿立即重发）"
    : "未抓到成功回包且未配置 Cookie 无法核对，已默认记为发布成功（请到「已发布帖子」确认，勿立即重发）";
  return {
    id: null,
    shareLink: null,
    publishStatus: "assumed_success",
    assumedSuccess: true,
    bodyTextOnly: String(text || "").trim() || undefined,
    message: tip,
...
export async function publishPost(apiKey, post, uploadsDir, onProgress, options = {}) {
  return withGlobalPublishQueue(async () => {
    ...
    try {
      throwIfAborted(signal);
      // options.proxyUrl：调用方传入的账号独立代理（或空=直连）
      const proxyUrl = options.proxyUrl || "";
      const fallbackProxyUrlRaw =
        options.fallbackProxyUrl !== undefined ? options.fallbackProxyUrl : getProxyUrl();
      // 独立远程 IP 默认清空 fallback，预检/发帖失败都不会改走本机全局
      const fallbackProxyUrl = shouldAllowProxyFallback(proxyUrl, fallbackProxyUrlRaw, options)
        ? fallbackProxyUrlRaw
        : "";
      const cookie = String(options.cookie || "").trim();
      // 仅显式开启时，回包丢失才记为成功（默认仍报「待确认」，避免误判成功率）
      const assumeSuccessOnUnknown = options.assumeSuccessOnUnknown === true;
      const skipPreflight = options.skipPreflight === true;
      const { text, title, imagePaths = [] } = post;
      if (!text?.trim()) throw new Error("帖子内容不能为空");

      // 实际请求出口：整次发帖 sticky 用这一个，避免中途换 IP
      let effectiveProxy = proxyUrl;

      if (!skipPreflight) {
        throwIfAborted(signal);
        // 独立代理偶发抖动：多试几次再判定失败
        const maxPreflightAttempts = isRemoteAccountProxy(effectiveProxy) ? 3 : 1;
        ...
        for (let attempt = 1; attempt <= maxPreflightAttempts; attempt++) {
          try {
            if (attempt > 1) {
              onProgress?.({
                stage: "network_check",
                message: `账号代理预检重试（${attempt}/${maxPreflightAttempts}）…`,
              });
              await sleepCancellable(800 * attempt, signal);
            }
            ...
            throwIfAborted(signal);
            lastPreflightErr = err;
          }
        }
        if (lastPreflightErr) {
          // fallbackProxyUrl 为空时（独立 IP）不会进这里，直接把预检错误抛给调用方
          if (canUseFallbackProxy(effectiveProxy, fallbackProxyUrl)) {
            onProgress?.({ stage: "network_check", message: "账号代理不通，改用全局代理预检…" });
            appendSystemLog(
              `[发帖] 账号代理预检失败，已改走全局代理（原代理可能未生效或不稳定）`,
              { type: "warn", source: "publish" },
            );
            await assertPublishNetworkReady(fallbackProxyUrl, { onProgress });
            effectiveProxy = fallbackProxyUrl;
          } else {
            if (isRemoteAccountProxy(proxyUrl)) {
              appendSystemLog(
                `[发帖] 账号独立代理预检失败，已禁止改走全局（请换可用节点后再发）`,
                { type: "err", source: "publish" },
              );
            }
            throw lastPreflightErr;
          }
        }
      }
      ...
      const recoverByFetch = async (causeErr) => {
        ...
        throwIfAborted(signal);
        if (assumeSuccessOnUnknown) {
          onProgress?.({
            stage: "publishing",
            message: "未抓到回包，已按设置默认记为发布成功（请勿立即重发）…",
          });
          appendSystemLog(
            `[发帖] 确认回包丢失，默认记为成功（${cookie ? "核对未命中" : "无 Cookie 已跳过核对"}）：${toChineseError(causeErr)}`,
            { type: "warn", source: "publish" },
          );
          return makeAssumedPublishSuccess(causeErr, {
            hasCookie: Boolean(cookie),
            text: body.bodyTextOnly,
          });
        }
        recordProxyFailure(effectiveProxy, causeErr);
        throw makeUncertainPublishError(causeErr, { hasCookie: Boolean(cookie) });
      };
      ...
      try {
        ...
        if (!result?.id) {
          ...
          if (assumeSuccessOnUnknown) {
            onProgress?.({
              stage: "publishing",
              message: "未拿到帖子 ID，已按设置默认记为发布成功（请勿立即重发）…",
            });
            appendSystemLog(
              `[发帖] 提交后无帖子 ID，默认记为成功（${cookie ? "核对未命中" : "无 Cookie"}）`,
              { type: "warn", source: "publish" },
            );
            return makeAssumedPublishSuccess(new Error("提交后未拿到帖子 ID"), {
              hasCookie: Boolean(cookie),
              text: body.bodyTextOnly,
            });
          }
          recordProxyFailure(effectiveProxy, new Error("未拿到帖子 ID"));
          const uncertain = makeUncertainPublishError(
            new Error("提交后未拿到帖子 ID，且广场记录中未核对到"),
            { hasCookie: Boolean(cookie) },
          );
        ...
        if (err?.code === "PUBLISH_CONFIRMATION_UNKNOWN") {
          if (err.alreadyConfirmed) {
            if (assumeSuccessOnUnknown) {
              onProgress?.({
                stage: "publishing",
                message: "确认状态未知，已按设置默认记为发布成功（请勿立即重发）…",
              });
              appendSystemLog(`[发帖] 确认未知，默认记为成功：${toChineseError(err)}`, {
                type: "warn",
                source: "publish",
              });
              return makeAssumedPublishSuccess(err, {
                hasCookie: Boolean(cookie),
                text: body.bodyTextOnly,
              });
            }
