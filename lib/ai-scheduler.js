import { publishPost, abortActivePublishRequests } from "./square-api.js";
import {
...
import { getProxyDisplayLabel, buildProxyUrl, isCustomProxyConfig } from "./proxy-config.js";
...
import {
  setAiRunProgress,
  scheduleClearAiRunProgress,
  beginAiRunAccounts,
  setAiAccountProgress,
} from "./ai-run-progress.js";
import {
...
/** 可被取消打断的等待；onTick 用于刷新「还需等待 Xs」进度 */
async function delayCancellable(ms, stepMs = 200, onTick = null) {
  const total = Math.max(0, Number(ms) || 0);
  const step = Math.max(50, Number(stepMs) || 200);
  if (total <= 0) return true;
  const totalSec = Math.max(1, Math.round(total / 1000));
  ...
  while (waited < total) {
    ...
    await delay(chunk);
    waited += chunk;
    const leftSec = Math.max(0, Math.ceil((total - waited) / 1000));
    if (leftSec > 0 || waited >= total) onTick?.(leftSec, totalSec);
  }
  return !cancelRequested;
}

/** 请求结束当前托管轮次（账号循环会尽快停下；进行中的 AI / 发帖核对也会打断） */
export function requestAiHostCancel() {
  cancelRequested = true;
  abortActiveAiRequest("已取消托管");
  abortActivePublishRequests("已取消托管");
  try {
...
function shouldRunNow(settings, now = Date.now()) {
  ...
  const intervalMs = resolveWaitIntervalMinutes(settings) * 60 * 1000;
...
function resolveHostedTargets(settings, overrides = {}) {
  ...
  // 定时任务：本轮覆盖启用账号（按轮转顺序）；「并行账号数」只控制同时跑几路
  const batchSize = Math.max(1, enabledHosted.length);
  return peekNextHostedBatch(settings, batchSize);
}
...
async function runHostedAccountCycle({
  ...
  for (let i = 0; i < runCount; i++) {
    ...
    setAiAccountProgress(accountId, {
    ...
    try {
      // —— 独立 IP 发帖入口 ——
      // 按 accountId 解析该账号自己的 Socks5/HTTP，禁止串到别的账号或本地全局
      const proxyUrl = resolveAccountProxy(accountId);
      const cookie = getAccountCookie(accountId);
      const proxyConfig = getAccountProxyConfig(accountId);
      const proxyLabel = getProxyDisplayLabel(proxyConfig);
      const resolvedHost = (() => {
        try {
          const built = proxyUrl || buildProxyUrl(proxyConfig);
          if (!built) return proxyConfig.type === "direct" ? "直连" : "全局/空";
      ...
      // 日志便于核对：Key / Cookie / 出口 IP 是否与页面配置一致
      appendSystemLog(
        `[AI托管] 发帖带入「${getAccountName(accountId)}」Key=${String(apiKey).slice(0, 8)}… Cookie=${cookie ? "有" : "无"} 代理=${proxyLabel} → 出口 ${resolvedHost}`,
        { type: "info", source: "ai-host" },
      );
      ...
      try {
        ...
        result = await publishPost(
          ...
          },
          {
            proxyUrl,
            cookie,
            // 自定义独立代理：失败也不改走本机 Clash；仅「使用全局代理」的账号才允许回落
            allowProxyFallback: !isCustomProxyConfig(proxyConfig),
            // AI 托管：回包丢失时仍记成功，便于统计代理抖动下的真实发出率（请勿立即重发）
            assumeSuccessOnUnknown: true,
          },
        );
        await delay(700);
      } catch (err) {
    ...
    } else if (result?.publishStatus === "assumed_success" || result?.assumedSuccess) {
      appendSystemLog(
        `[AI托管] 「${getAccountName(accountId)}」未抓到回包，已默认记为发布成功（请勿立即重发，可到广场核对）`,
        { type: "warn", source: "ai-host" },
      );
    }
    ...
    } else if (result?.assumedSuccess || result?.publishStatus === "assumed_success") {
      // 无真实 ID：不要写入 published-posts-cache（假 ID 会污染历史拉取）
      appendSystemLog(
        `[AI托管] 「${getAccountName(accountId)}」回包丢失已记为待核对，未写入历史缓存（请拉历史对账，勿重发）`,
        { type: "warn", source: "ai-host" },
      );
    } else if (result) {
      appendSystemLog(
    ...
    if (i < runCount - 1) {
      const gapMs = getPublishDelayMs(settings);
      const gapSec = Math.max(1, Math.round(gapMs / 1000));
      appendSystemLog(
        `[AI托管] 「${getAccountName(accountId)}」条间间隔生效：等待 ${gapSec} 秒后再发下一条`,
        { type: "info", source: "ai-host" },
      );
      await delayCancellable(gapMs, 1000, (leftSec, totalSec) => {
        setAiRunProgress(
          "waiting",
          leftSec > 0
            ? `条间间隔：还需等待 ${leftSec}/${totalSec} 秒（「${getAccountName(accountId)}」）…`
            : `条间间隔结束，继续发帖…`,
        );
      });
...
export async function runAiHostedCycle({
  ...
  beginAiRunAccounts(
    targets.map((item) => ({
      ...
      name: getAccountName(item.accountId),
    })),
  );
  setAiRunProgress(
    "start",
    targets.length > 1
      ? `已开始：${targets.length} 个账号，并行 ${Math.min(concurrency, targets.length)} 路写稿（各账号独立代理）…`
      : "已开始：准备托管任务…",
  );
  ...
  try {
    await runPool(targets, concurrency, async (hostConfig, index) => {
      ...
      if (cancelRequested) {
        setAiAccountProgress(accountId, { name: accountName, status: "cancelled", detail: "已取消" });
        return;
      }
      if (!canAccountRunAiToday(readAiSettings(), accountId)) {
        setAiAccountProgress(accountId, {
      ...
      // 错开启动：多路并行时削峰；单路串行时账号之间稍作间隔，减轻 VPN 连续发帖抖动
      if (index > 0) {
        await delay(concurrency > 1 ? Math.min(800, 120 * index) : 1200);
      }
      setAiAccountProgress(accountId, { name: accountName, status: "running", detail: "写稿中" });
      setAiRunProgress(
        "write",
        `并行 ${Math.min(concurrency, targets.length)} 路 · 账号 ${index + 1}/${targets.length}「${accountName}」…`,
      );
      try {
        ...
        const publishedDelta = published.length - publishedBefore;
        const generatedDelta = generated.length - generatedBefore;
        if (shouldPublish) {
          if (publishedDelta > 0) {
            setAiAccountProgress(accountId, {
              name: accountName,
              status: "success",
              detail: `已发 ${publishedDelta} 条`,
              error: "",
            });
          } else if (generatedDelta > 0) {
            setAiAccountProgress(accountId, {
              name: accountName,
              status: "skipped",
              detail: "已生成未发布",
        ...
        // 仅用户主动取消才标「取消」；网络抖动应记为失败
        const cancelled =
          cancelRequested ||
          err?.code === "HOST_CANCELLED" ||
          /已取消托管/.test(zh) ||
          /已取消托管/.test(String(err?.message || ""));
        if (cancelled) {
          setAiAccountProgress(accountId, {
            name: accountName,
            status: "cancelled",
          ...
          appendSystemLog(`[AI托管] 账号「${accountName}」因取消而中止`, {
            type: "info",
            source: "ai-host",
          });
          return;
        }
        accountErrors.push({ accountId, error: zh });
        setAiAccountProgress(accountId, {
          name: accountName,
          status: "failed",
          error: zh,
        ...
        appendSystemLog(`[AI托管] 账号「${accountName}」失败：${zh}`, {
          type: "err",
          source: "ai-host",
        });
      }
