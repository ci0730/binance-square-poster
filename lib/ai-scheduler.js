...
/** 多账户发帖串行，降低广场 429 / 代理打满 */
let publishQueue = Promise.resolve();

function withPublishQueue(fn) {
  const run = publishQueue.then(fn, fn);
...
async function runHostedAccountCycle({
  ...
  for (let i = 0; i < runCount; i++) {
    ...
    let result;
    try {
      // 发帖全局排队：写稿可并行，提交广场串行，减少限流
      result = await withPublishQueue(async () => {
        try {
