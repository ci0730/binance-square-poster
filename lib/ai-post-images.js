...
/** 同币 K 线短缓存，多账户配图时避免重复打币安 */
const KLINE_CACHE_MS = 3 * 60 * 1000;
const klineCloseCache = new Map();
const klineCloseInflight = new Map();
...
async function fetchClosePrices(symbol) {
  ...
  const pair = `${sym}USDT`;
  const cacheKey = `${pair}:1h:48`;
  const cached = klineCloseCache.get(cacheKey);
  if (cached && Date.now() - cached.at < KLINE_CACHE_MS && cached.closes?.length) {
    return cached.closes;
  }
  if (klineCloseInflight.has(cacheKey)) {
    return klineCloseInflight.get(cacheKey);
  }
  ...
  const task = (async () => {
    ...
    try {
      const closes = await Promise.any(attempts);
      klineCloseCache.set(cacheKey, { at: Date.now(), closes });
      return closes;
    } catch {
      return null;
    } finally {
      klineCloseInflight.delete(cacheKey);
    }
  })();

  klineCloseInflight.set(cacheKey, task);
  return task;
}
