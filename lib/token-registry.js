...
const DEFAULT_CHAINS = [
  ...
  { id: "binance-alpha", label: "币安 Alpha / 合约" },
  { id: "bsc", label: "BSC" },
...
function normalizeListingType(value, chain = "") {
  if (value === "alpha" || value === "spot") return value;
  return chain === "binance-alpha" ? "alpha" : "spot";
}

function normalizeEntry(raw = {}) {
  const symbol = normalizeSymbol(raw.symbol);
  if (!symbol) return null;
  const source = ["auto", "spot", "alpha"].includes(raw.source) ? raw.source : "auto";
...
export function listTokenRegistryPublic() {
  ...
  return {
    ...
    sourceOptions: [
      { id: "auto", label: "自动（现货优先，失败再 Alpha）" },
      { id: "spot", label: "仅币安现货" },
  ...
  for (const raw of syncedItems) {
    ...
    if (!existing) {
      const created = defaultEntry(symbol, {
        ...
        listingType: raw.listingType || (raw.chain === "binance-alpha" ? "alpha" : "spot"),
        chain: raw.chain || (incomingContract && raw.listingType === "alpha" ? "binance-alpha" : "binance-spot"),
