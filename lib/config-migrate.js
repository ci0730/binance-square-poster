...
export function listDefaultLegacyDataDirs({ installDataDir = null } = {}) {
  ...
  // 安装版桌面端默认数据目录（userData）
  const electronUserData = process.env.APPDATA
    ? path.join(process.env.APPDATA, "binance-square-poster")
    : null;
  if (installDataDir) dirs.push(installDataDir);
  if (electronUserData) dirs.push(electronUserData);
