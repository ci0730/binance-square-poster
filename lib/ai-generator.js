...
import { buildGeminiUrl, resolveProviderRuntime, toOpenAiModelsUrl, AUTO_MATCH_MODEL_CANDIDATES } from "./ai-providers.js";
...
function isModelUnsupportedError(err) {
  const msg = String(err?.message || err || "");
  return /not supported by any configured account|no available channel|无可用渠道|模型.*不支持|Group .* not|分组不支持|接口返回 404|接口不存在（404）|model_not_found|does not exist|unknown model/i.test(
    msg,
  );
}
...
async function buildAutoMatchCandidates(apiKey, baseUrl, preferredModel = "") {
  const preferred = String(preferredModel || "").trim();
  const remote = await fetchRemoteModelIds(apiKey, baseUrl);
  const ordered = [];
  const push = (id) => {
...
export async function testAiApiKey(
  ...
  if (preferred) {
    try {
      ...
      if (!shouldAutoMatch || isFatalAuthError(err) || !isModelUnsupportedError(err)) {
        throw err;
      }
    }
  ...
  const candidates = await buildAutoMatchCandidates(apiKey, resolvedBaseUrl, preferred);
  const tried = [];
  for (const candidate of candidates) {
    ...
    try {
      ...
      tried.push({ model: candidate, error: String(err.message || err).slice(0, 120) });
    }
  }
  ...
  const triedText = tried.length
    ...
    : "（无候选）";
  throw new Error(
    `自动匹配失败：当前 Key 分组下未找到可用模型。已尝试：${triedText}。请到服务商后台查看该分组支持的模型名后手动填写。`,
  );
}
