...
export function abortActiveAiRequest(reason = "已取消托管") {
  ...
  for (const req of [...activeAiRequests]) {
    activeAiRequests.delete(req);
    try {
      req.destroy(err);
    } catch {
      // ignore
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          finish(resolve, { status: res.statusCode, data, text });
        });
        res.on("aborted", () => finish(reject, new Error("AI 请求已中断")));
      },
    );
    activeAiRequests.add(req);
    req.on("timeout", () => req.destroy(new Error("AI 请求超时")));
    req.on("error", (err) => finish(reject, err));
    req.on("close", () => {
      if (!settled) finish(reject, new Error("AI 连接已关闭"));
    });
