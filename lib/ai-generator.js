...
function shouldTryNextModel(err) {
  ...
  const msg = String(err?.message || err || "");
  // 余额/限流/网络问题不盲试其它模型
  if (/429|quota|余额|insufficient|rate limit|超时|timeout|ECONN|ENOTFOUND|network|证书|TLS|代理/i.test(msg)) {
    return false;
  }
  return /not supported by any configured account|no available channel|无可用渠道|模型.*不支持|Group .* not|分组不支持|接口返回 404|接口不存在（404）|model_not_found|does not exist|unknown model|invalid model|模型不存在|模型不可用|400|404|invalid_request|Parameter .+ model/i.test(
    msg,
  );
}
...
async function buildAutoMatchCandidates(apiKey, baseUrl, preferredModel = "", providerId = "") {
  ...
  if (useRemoteFallback) {
    const remote = await fetchRemoteModelIds(apiKey, baseUrl || provider.baseUrl);
    remote.forEach(push);
