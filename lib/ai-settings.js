...
export function recordAiRun({ success, error, tokenPair = null } = {}) {
  ...
  if (success) {
    ...
    // 部分账号失败时保留 error，便于状态栏简要展示原因；全成功则清空
...
/** 发布前同步占用今日额度；失败返回 false，避免并行超发 */
export function tryClaimTodayPublishSlot(count = 1) {
  ...
  if (settings.todayPublished + n > settings.maxPostsPerDay) return false;
  settings.todayPublished += n;
  settings.totalPublished += n;
  writeSettings(settings);
  return true;
...
export function releaseTodayPublishSlot(count = 1) {
  const n = Math.max(1, parseInt(count, 10) || 1);
  const settings = readAiSettings();
  if (settings.todayDate !== todayKey()) return settings.todayPublished;
  settings.todayPublished = Math.max(0, (settings.todayPublished || 0) - n);
