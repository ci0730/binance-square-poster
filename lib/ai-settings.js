...
import { DEFAULT_CONTENT_STYLES, normalizeContentStyles, CONTENT_STYLE_OPTIONS } from "./ai-content-styles.js";
...
export function resolveAiCredentials({ profileId, hostConfig, settings = readAiSettings(), overrides = {} } = {}) {
  ...
  const provider = normalizeAiProviderId(overrides.provider || profile?.provider || settings.provider);
  const baseUrl = overrides.baseUrl ?? profile?.baseUrl ?? settings.baseUrl ?? "";
  const model = normalizeAiModel(provider, overrides.model || profile?.model || settings.model, baseUrl);
...
function normalizeHostedAccount(raw = {}, legacyDefaults = {}) {
  ...
  return {
    ...
    contentStyles: normalizeContentStyles(raw.contentStyles ?? legacyDefaults.contentStyles ?? DEFAULT_CONTENT_STYLES),
  };
}
...
function migrateHostedAccounts(settings) {
  if (Array.isArray(settings.hostedAccounts) && settings.hostedAccounts.length) {
    return settings.hostedAccounts
      .map((item) => normalizeHostedAccount(item))
      .filter(Boolean);
  }
  ...
  if (!accountId) return [];
  const migrated = normalizeHostedAccount({ accountId, enabled: true, ...legacyDefaults }, legacyDefaults);
  return migrated ? [migrated] : [];
}

function syncHostedAccountsWithAccountList(hostedAccounts, accounts) {
  const map = new Map(hostedAccounts.map((item) => [item.accountId, item]));
  return accounts
    .map((acc) => {
      const existing = map.get(acc.id);
      if (existing) return normalizeHostedAccount(existing);
      return normalizeHostedAccount({ accountId: acc.id, enabled: false });
    })
    .filter(Boolean);
}

export function getEnabledHostedAccounts(settings = readAiSettings()) {
  return syncHostedAccountsWithAccountList(migrateHostedAccounts(settings), listAccountsPublic().accounts).filter(
    (item) => item.enabled,
  );
}
...
export function getHostedAccountConfig(accountId, settings = readAiSettings()) {
  ...
  const hosted = syncHostedAccountsWithAccountList(migrateHostedAccounts(settings), accounts);
...
function normalizeSettings(raw = {}) {
  ...
  settings.publishIntervalSeconds = Math.max(1, Math.min(parseInt(settings.publishIntervalSeconds, 10) || 3, 60));
  ...
  settings.contentStyles = normalizeContentStyles(settings.contentStyles);
  ...
  settings.hostedAccounts = syncHostedAccountsWithAccountList(migrateHostedAccounts(settings), accounts);
...
export function getAiSettingsPublic() {
  ...
  const accounts = listAccountsPublic().accounts;
  const hostedAccounts = settings.hostedAccounts.map((item) => {
  ...
  return {
    ...
    allowCustomModel: Boolean(provider.allowCustomModel),
    contentStyles: settings.contentStyles,
    contentStyleOptions: CONTENT_STYLE_OPTIONS,
    selectedTokens: settings.selectedTokens,
    customTokens: settings.customTokens,
    marketSentiment: settings.marketSentiment,
...
export function recordAiRun({ success, error, published = 0, tokenPair = null } = {}) {
  const settings = readAiSettings();
  settings.lastRunAt = Date.now();
  if (settings.todayDate !== todayKey()) {
    settings.todayDate = todayKey();
  ...
  if (success) {
    ...
    settings.todayPublished += published;
    settings.totalPublished += published;
